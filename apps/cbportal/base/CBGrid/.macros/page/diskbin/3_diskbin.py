def main(j, args, params, tags, tasklet):
    ...
    def remove_icon(row, field):
        ...
        return page.getActionHtml(
            "action-DestroyDisk", tags, "glyphicon glyphicon-remove"
        )

    def restore_icon(row, field):
        tags = {"data-diskId": row[field]}
        return page.getActionHtml(
            "action-RestoreDisk", tags, "glyphicon glyphicon-repeat"
        )
    ...
    tableid = modifier.addTableFromModel(
        "cloudbroker", "disk", fields, filters, selectable="rows", tableid=tableid
    )
