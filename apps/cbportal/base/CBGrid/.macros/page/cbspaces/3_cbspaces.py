def main(j, args, params, tags, tasklet):
    ...
    def makeNetworkLink(row, field):
        if row[field]:
            return (
                "[%(networkId)s|/CBGrid/private network?id=%(networkId)s&gid=%(gid)s]"
                % row
            )
        else:
            return ""
    ...
    def makeExternalNetworkLink(row, field):
        if row[field]:
            return (
                "[%(externalnetworkip)s|/CBGrid/External Network?networkid=%(externalnetworkId)s]"
                % row
            )
        else:
            return ""
    ...
    fields = [
        {"name": "ID", "value": "[%(id)s|/CBGrid/Cloud Space?id=%(id)s]", "id": "id"},
        ...
        {"name": "Network ID", "value": makeNetworkLink, "id": "networkId"},
        {"name": "Location Code", "value": "location", "id": "location"},
    ...
    tableid = modifier.addTableFromModel(
        "cloudbroker", "cloudspace", fields, filters, selectable="rows"
    )
