def main(j, args, params, tags, tasklet):
    ...
    def remove_icon(row, field):
        ...
        return page.getActionHtml(
            "action-DestroyImage", tags, "glyphicon glyphicon-remove"
        )

    def restore_icon(row, field):
        tags = {"data-imageId": row[field]}
        return page.getActionHtml(
            "action-RestoreImage", tags, "glyphicon glyphicon-repeat"
        )
    ...
    tableid = modifier.addTableFromModel(
        "cloudbroker", "image", fields, filters, selectable="rows"
    )
