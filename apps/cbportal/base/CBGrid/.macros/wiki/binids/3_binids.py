def main(j, args, params, tags, tasklet):
    ...
    cloudspaceIds = _get_data(
        ccl.cloudspace.search({"$fields": ["id"], "$query": {"status": "DELETED"}})[1:]
    )
    machineIds = _get_data(
        ccl.vmachine.search({"$fields": ["id"], "$query": {"status": "DELETED"}})[1:]
    )
    BdiskIds = _get_data(
        ccl.disk.search(
            {
    ...
    imageIds = _get_data(
        ccl.image.search({"$fields": ["id"], "$query": {"status": "DELETED"}})[1:]
    )
    accountIds = _get_data(
        ccl.account.search({"$fields": ["id"], "$query": {"status": "DELETED"}})[1:]
    )
