...
class cloudapi_s3storage(BaseActor):
    ...
    def __init__(self):
        ...
        self.osis_logs = j.clients.osis.getCategory(
            j.core.portal.active.osis, "system", "log"
        )

    def _get(self, cloudspaceId):
        cloudspace = self.models.cloudspace.get(int(cloudspaceId))
        s3storagebuckets = self.models.s3user.search(
            {"cloudspaceId": int(cloudspaceId), "location": cloudspace.location}
        )[1:]
