...
class SSHMngr(object):
    ...
    def reflector(self):
        if self._reflectorService is None:
            self._reflectorService = self._getService(
                self._config.getStr("instance.reflector.name")
            )
...
class bootstrap(Resource):
    def lightweight(self, hostname, data):
        ...
        masterPub = j.system.fs.fileGetContents(
            j.system.fs.joinPaths(args.gitpath, "keys/master_root.pub")
        )
        gitPub = j.system.fs.fileGetContents(
            j.system.fs.joinPaths(args.gitpath, "keys/git_root.pub")
        )
        gitPriv = j.system.fs.fileGetContents(
            j.system.fs.joinPaths(args.gitpath, "keys/git_root")
        )

        environment = data.get("environment")
        if not environment:
            setuphrd = j.application.getAppInstanceHRD(
                "ovc_setup", "main", "openvcloud"
            )
            environment = setuphrd.get("instance.ovc.environment")

        # make location
        location = next(
            iter(j.atyourservice.findServices(name="location", instance=environment)),
            None,
        )
        if not location:
        ...
        try:
            ...
            data = {"instance.key.priv": gitPriv, "instance.key.pub": gitPub}
            ...
            node = j.atyourservice.new(
                name="node.ssh", instance=hostname, args=data, parent=location
            )
            ...
            return self.error(
                500, "error during creation of the node.ssh service: %s" % e.message
            )
    ...
    def post(self):
        ...
        try:
            ...
            return self.error(
                500,
                "Error during pushing of the public key to reflector: %s" % e.message,
            )
        ...
        if not j.system.fs.exists(path=masterKeyPath) or not j.system.fs.exists(
            path=gitKeyPath
        ):
            j.atyourservice.remove(name="sshkey", instance=hostname)
            j.atyourservice.remove(name="node.ssh", instance=hostname)
        ...
        autossh = j.atyourservice.new(
            name="autossh", instance=hostname, args=data, parent=node
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gitpath", default="/opt/code/OVC_GIT", help="path to the OVC_GIT repo\n"
    )
