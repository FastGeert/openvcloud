...
class SSHConfigGenerator:
    ...
    def identityfile(self):
        if self._identityfile is None:
            self._identityfile = os.path.abspath(
                os.path.join(self.rootpath, "keys", "git_root")
            )
    ...
    def print_host(self, host, reflector=None, reflectorport=None):
        ...
        print("  IdentityFile %s" % self.identityfile)
        print(
            "  ProxyCommand ssh -A -i %s -q root@%s -p %s nc -q0 %%h %%p"
            % (self.identityfile, self.host, self._controller)
        )
        print("")
