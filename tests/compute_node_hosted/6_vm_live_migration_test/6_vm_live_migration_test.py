...
def main():
    ...
    try:
        if not j.system.net.waitConnectionTest(
            cloudspace_publicip, cloudspace_publicport, 40
        ):
        ...
        for i in range(2):
            if i == 0:
                ...
                p = multiprocessing.Process(
                    target=pcl.actors.cloudbroker.machine.moveToDifferentComputeNode,
                    args=(machineId, "Testing", stacks[1], False),
                )
                processes.append(p)
...
if __name__ == "__main__":
    try:
        ...
        j.do.execute(
            "jspython performance_testing/scripts/tear_down.py vmlivemigrateuser"
        )
        try:
            if test_result == "Two files are identical":
                print ("################ \n# Test succeed #\n################")
