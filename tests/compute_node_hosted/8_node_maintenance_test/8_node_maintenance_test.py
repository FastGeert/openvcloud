...
def main():
    ...
    utils.setup_machine(
        cloudspace, machineId, cloudspace_publicport, pcl, no_of_disks, "onlyfio"
    )
    ...
    for i in range(2):
        if i == 0:
            p = multiprocessing.Process(
                target=utils.run_script,
                args=(account, cloudspace_publicip, cloudspace_publicport, "test1"),
            )
            processes.append(p)
        else:
    ...
    test_result = utils.check_script(
        account, cloudspace_publicip, cloudspace_publicport, "test1.1.0", "test2.1.0"
    )
...
if __name__ == "__main__":
    try:
        ...
        j.do.execute(
            "jspython performance_testing/scripts/tear_down.py nodemaintenanceuser"
        )
        try:
            pcl = j.clients.portal.getByInstance("main")
            pcl.actors.cloudbroker.computenode.enable(
                id=stackid, gid=gid, message="testing"
            )
            if test_result == "Two files are identical":
                print("################ \n# Test succeed #\n################")
            else:
