...
def main(snapshots_number):
    ...
    for i in range(snapshots_number):
        ...
        try:
            pcl.actors.cloudapi.machines.snapshot(
                machineId=machineId, name="snapshot%s" % (i + 1)
            )
            ...
            snapshots = utils.run_again_if_failed(
                pcl.actors.cloudapi.machines.listSnapshots, machineId=machineId
            )
            print("snapshots length: %s" % len(snapshots))
            continue
        time.sleep(5)
...
if __name__ == "__main__":
    try:
        ...
        j.do.execute(
            "jspython performance_testing/scripts/tear_down.py vmsnapshotsuser"
        )
