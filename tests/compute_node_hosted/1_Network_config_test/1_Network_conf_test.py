...
for stackid in stacks:
    ...
    machineId = utils.create_machine_onStack(
        stackid,
        cloudspace,
        0,
        ccl,
...
for vm in machines:
    ...
    j.do.execute(
        "sshpass -p%s scp -o 'StrictHostKeyChecking=no' -P %s noerror.txt  %s@%s:"
        % (
            account["password"],
            cloudspace_publicport,
            account["login"],
    ...
    j.do.execute(
        "sshpass -p%s scp -o 'StrictHostKeyChecking=no' -P %s functional_testing/Openvcloud/compute_node_hosted/1_Network_config_test/machine_script.py  %s@%s:"
        % (
            account["password"],
            cloudspace_publicport,
            account["login"],
...
for vm in machines:
    ...
    for vmach in machines:
        ...
        connection.run(
            "sshpass -p%s scp -o 'StrictHostKeyChecking=no' noerror%s.txt  %s@%s:"
            % (account["password"], vmId, account["login"], machine_ip)
        )
        connection1 = j.remote.cuisine.connect(
            cloudspace_publicIP,
            cloudspace_publicport,
            account["password"],
            account["login"],
...
for vm in machines:
    ...
    f = open("final.txt", "a")
    j.do.execute(
        "sshpass -p%s scp -r -o 'StrictHostKeyChecking=no' -P %s  %s@%s:results.txt ."
        % (
            account["password"],
            cloudspace_publicport,
            account["login"],
