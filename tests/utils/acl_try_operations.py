...
def try_account_read(self, operation="get"):
    if operation == "get":
        ...
        user_account = self.user_api.cloudapi.accounts.getConsumedCloudUnits(
...
def try_account_write(self, operation="create_cloudspace"):
    if operation == "create_cloudspace":
        ...
        newcloudspaceId = self.account_owner_api.cloudapi.cloudspaces.create(
            accountId=self.account_id,
            location=self.location,
        ...
        converted = self.user_api.cloudapi.machines.convertToTemplate(
            machineId=machineId, templatename=str(uuid.uuid4()).replace("-", "")[0:10]
        )
...
def try_account_admin(self, operation="add_user"):
    if operation == "add_user":
        ...
        self.user_api.cloudapi.accounts.update(
            accountId=self.account_id, name=self.user
        )
...
def try_cloudspace_read(self, operation="getCloudspace"):
    if operation == "getCloudspace":
        ...
        owner_vms = self.account_owner_api.cloudapi.machines.list(
            cloudspaceId=self.cloudspace_id
        )
        user_vms = self.user_api.cloudapi.machines.list(cloudspaceId=self.cloudspace_id)
        if len(owner_vms) != len(user_vms):
            [
                self.user_api.cloudapi.machines.get(machineId=vm["id"])
                for vm in owner_vms
            ]
        ...
        self.assertEqual(
            ...
            "cloudspace [%s] owned by user [%s]"
            % (self.user, self.cloudspace_id, self.account_owner),
        )
    else:
        raise AssertionError("Un-supported operation [%s]" % operation)
...
def try_cloudspace_write(self, operation="cloudspaceDeploy"):
    if operation == "cloudspaceDeploy":
        ...
        self.lg("- deploy cloudspace with new user [user], should succeed")
        self.account_owner_api.cloudapi.cloudspaces.deploy(
            cloudspaceId=self.cloudspace_id
        )
        self.wait_for_status(
            "DEPLOYED",
        ...
        defense_shield = self.user_api.cloudapi.cloudspaces.getDefenseShield(
            cloudspaceId=self.cloudspace_id
        )
        [
...
def try_cloudspace_admin(self, operation="cloudspaceAdduser"):
    if operation == "cloudspaceAdduser":
        ...
        self.lg("- add user3 to the cloudspace by user2 with read access")
        self.add_user_to_cloudspace(
            cloudspace_id=self.cloudspace_id, user=user3, accesstype="R"
        )
        self.lg("- update user3 access on cloudspace by user2 with write access")
        self.user_api.cloudapi.cloudspaces.updateUser(
            cloudspaceId=self.cloudspace_id, userId=user3, accesstype="CRX"
        )
        ...
        self.assertEqual(len(cloudspaces), 1, "Should have only the default cloudspace")
    else:
        raise AssertionError("Un-supported operation [%s]" % operation)
...
def try_machine_write(self, operation="start_machine"):
    if operation == "start_machine":
        ...
        name = str(uuid.uuid4()).replace("-", "")[0:10]
        self.user_api.cloudapi.machines.snapshot(machineId=self.machine_id, name=name)
        ...
        self.user_api.cloudapi.machines.rollbackSnapshot(
            machineId=self.machine_id, epoch=snapshots[0]["epoch"]
        )
