...
class AccountsTests(Framework):
    ...
    def test01_edit_account(self):
        ...
        **Test Scenario:**
        ...
        self.Accounts.create_new_account(
            self.account, self.admin_username + "@itsyouonline"
        )
    ...
    def test03_add_account_with_decimal_limitations(self):
        ...
        **Test Scenario:**
        ...
        max_memory = "3.5"
        self.Accounts.create_new_account(
            self.account, self.admin_username + "@itsyouonline", max_memory=max_memory
        )
        self.Accounts.open_account_page(self.account)
        account_maxmemory = self.get_text("account_page_maxmemory")
        self.assertTrue(
            account_maxmemory.startswith(max_memory),
            "Account max memory is [%s]"
