...
class QuickTutorialGuide(Framework):
    ...
    def test01_intro(self):
        ...
        **Test Scenario:**
        ...
        self.assertEqual(self.get_text("intro_subheader_label"), "Questions?")
        self.assertEqual(
            self.get_text("intro_line5_label"),
            "In case of any questions, go to our Documentation Center or "
            "contact Support",
        )
        self.assertEqual(
            self.element_link("intro_spport_link"),
    ...
    def test02_cloudspace(self):
        ...
        **Test Scenario:**
        ...
        self.assertEqual(self.get_text("cloudspace_header_label"), "Cloud Space")
        self.assertEqual(
            self.get_text("cloudspace_line1_label"),
            "Optionally Go to Cloud Space Settings to provide access to other "
            "registered users in the OpenvCloud cloud.",
        )
        self.assertEqual(
            self.get_text("cloudspace_line2_label"),
        ...
        self.assertEqual(self.get_text("cloudspace_subheader_label"), "Questions?")
        self.assertEqual(
            self.get_text("cloudspace_line3_label"),
            "In case of any questions, go to our Documentation Center or contact "
            "Support",
        )
        self.assertEqual(
            self.element_link("cloudspace_spport_link"),
            "%s/g8vdc/#/Support" % self.environment_url.replace("http:", "https:"),
        )
        self.lg("%s ENDED" % self._testID)
    ...
    def test03_machines(self):
        ...
        **Test Scenario:**
        ...
        self.assertEqual(self.get_text("machines_header_label"), "Machines")
        self.assertEqual(
            self.get_text("machines_line1_label"),
            "Click on Machines, hit Create Machine",
        )
        self.assertEqual(
            self.get_text("machines_line2_label"),
            "Set the name for your Virtual Machine",
        )
        self.assertEqual(
            self.get_text("machines_line3_label"),
            "Choose one of the packages, delivering CPU, RAM and 10GB SSD",
        )
        self.assertEqual(self.get_text("machines_line4_label"), "Expand your SSD size")
        self.assertEqual(self.get_text("machines_line5_label"), "Hit Create")
        self.assertEqual(self.get_text("machines_line6_label"), "Go to Console tab")
        self.assertEqual(
            self.get_text("machines_line7_label"),
            "Login and take control (Windows Images need to load some minutes "
            "to finish all initalisations before you can login)",
        )
        self.assertEqual(
            self.get_text("machines_line8_label"),
        ...
        self.assertEqual(
            self.get_text("machines_line9_label"),
        ...
        self.assertEqual(
            self.get_text("machines_line10_label"),
            "Your virtual machine is charged once created per hour, for minimal "
            "an hour. You can delete anytime and the charging stops.",
        )
        self.assertEqual(self.get_text("machines_subheader_label"), "Questions?")
        self.assertEqual(
            self.get_text("machines_line11_label"),
            "In case of any questions, go to our Documentation Center or contact "
            "Support",
        )
        self.assertEqual(
            self.element_link("machines_spport_link"),
            "%s/g8vdc/#/Support" % self.environment_url.replace("http:", "https:"),
        )
        self.lg("%s ENDED" % self._testID)
    ...
    def test04_defense_shield(self):
        ...
        **Test Scenario:**
        ...
        self.assertEqual(self.get_text("defense_shield_header_label"), "Defense Shield")
        self.assertEqual(
            self.get_text("defense_shield_line1_label"), "Go to Defense Shield"
        )
        self.assertEqual(
            self.get_text("defense_shield_line2_label"),
            "Login with user 'admin' and your portal password",
        )
        self.assertEqual(
            self.get_text("defense_shield_line3_label"),
            "Configure what you like, it`s private it`s yours",
        )
        self.assertEqual(
            self.get_text("defense_shield_line4_label"),
            "Each Cloud Space has its own Defense Shield",
        )
        self.assertEqual(
            self.get_text("defense_shield_line5_label"),
            "For more simple tasks, use the Port Fowards in the Machines submenu",
        )
        self.assertEqual(self.get_text("defense_shield_subheader_label"), "Questions?")
        self.assertEqual(
            self.get_text("defense_shield_line6_label"),
            "In case of any questions, go to our Documentation Center or contact "
            "Support",
        )
        self.assertEqual(
            self.element_link("defense_shield_spport_link"),
            "%s/g8vdc/#/Support" % self.environment_url.replace("http:", "https:"),
        )
        self.lg("%s ENDED" % self._testID)
