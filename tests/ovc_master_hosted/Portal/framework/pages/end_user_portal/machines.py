...
class machines:
    ...
    def end_user_create_virtual_machine(
        self, image_name="Ubuntu 16.04 x64", machine_name=""
    ):
        ...
        self.framework.lg(
            "Create a machine name: %s image:%s" % (machine_name, image_name)
        )
        ...
        num_available_packages = len(
            self.framework.find_element("packages").find_elements_by_tag_name("li")
        )
        randome_package = randint(1, num_available_packages)
        self.framework.click_item("package", [randome_package])
        time.sleep(2)

        num_available_disk_sizes = len(
            self.framework.find_element("disk_sizes").find_elements_by_tag_name(
                "button"
            )
        )
        ...
        if self.framework.check_element_is_exist("end_user_machine_table"):
            ...
            for row in machine_table_rows:
                ...
                if machine_name == items[1].text:
                    ...
                    for _ in range(3):
                        if not self.framework.check_element_is_exist(
                            "end_user_machine_name"
                        ):
                            time.sleep(1)
                        else:
                            break
                    self.framework.assertIn(
                        machine_name,
                        self.framework.get_text("end_user_machine_name"),
    ...
    def end_user_get_machine_info(self, machine_name=""):
        ...
        if self.framework.check_element_is_exist("machine_description"):
            self.framework.machine_description = self.framework.get_text(
                "machine_description"
            )
        else:
        ...
        self.framework.machine_credentials = self.framework.get_text(
            "machine_credentials"
        )
    ...
    def end_user_verify_machine_elements(self, status):
        ...
        self.framework.assertEqual(
            self.framework.machine_ipaddress,
            self.framework.get_text("machine_ipaddress"),
        )
        self.framework.assertEqual(
            self.framework.machine_osimage, self.framework.get_text("machine_osimage")
        )
        self.framework.assertEqual(
            self.framework.machine_credentials,
            self.framework.get_text("machine_credentials"),
        )
        self.framework.assertEqual(
            self.framework.machine_cpu, self.framework.get_text("machine_cpu")
        )
        self.framework.assertEqual(
            self.framework.machine_memory, self.framework.get_text("machine_memory")
        )
        self.framework.assertEqual(
            self.framework.machine_storage, self.framework.get_text("machine_storage")
        )

    def end_user_verify_machine_console(self, status):
        if status == "RUNNING":
            self.framework.assertEqual(
                self.framework.get_text("console_message_running"),
            ...
            self.framework.assertEqual(
                self.framework.get_text("console_ipaddress"),
                self.framework.machine_ipaddress,
            )
            self.framework.assertTrue(
                self.framework.find_element("capture_button").is_displayed()
            )
            self.framework.assertTrue(
                self.framework.find_element("send_ctrl/alt/del_button").is_displayed()
            )
        else:
            self.framework.assertEqual(
                self.framework.get_text("console_message_halted"),
                "A machine must be started to access the console!",
            )
            self.framework.assertFalse(
                self.framework.find_element("capture_button").is_displayed()
            )
            self.framework.assertFalse(
                self.framework.find_element("send_ctrl/alt/del_button").is_displayed()
    ...
    def end_user_delete_virtual_machine(self, virtual_machine):
        ...
        if self.framework.check_element_is_exist("machines_button"):
            ...
            if self.framework.check_element_is_exist("end_user_machine_table"):
                ...
                machine_table = self.framework.find_element("end_user_machine_table")
                machine_table_rows = machine_table.find_elements_by_class_name(
                    "ng-scope"
                )

                for counter in range(len(machine_table_rows)):
                    machine_name_xpath = self.framework.elements[
                        "end_user_machine_name_table"
                    ][1] % (counter + 1)
                    machine_name = self.framework.driver.find_element_by_xpath(
                        machine_name_xpath
                    )
                    ...
                    self.framework.lg(
                        "can't find %s machine in the table" % virtual_machine
                    )
                    return False
                ...
                time.sleep(10)
                if self.framework.get_text(
                    "machine_list"
                ) == "Machines" or self.framework.element_is_displayed(
                    "no_machines_message"
                ):
                    return True
                else:
                    self.framework.lg(
                        "FAIL : Can't delete %s machine" % virtual_machine
                    )
                    return False
            else:
        ...
        if self.framework.check_element_is_exist("end_user_selected_account"):
            ...
            if account not in self.framework.get_text("end_user_selected_account"):
                ...
                for temp in range(100):
                    try:
                        account_item = self.framework.driver.find_element_by_xpath(
                            accounts_xpath % temp
                        )
                    except:
                        self.framework.lg(
                            "Can't choose %s account from the end user" % account
                        )
                        return False
                    else:
                        if account in account_item.text:
                            account_item.click()
                            cloud_space_xpath = (
                                self.framework.elements["end_user_cloud_space"][1]
                                % account
                            )
                            self.framework.driver.find_element_by_xpath(
                                cloud_space_xpath
                            ).click()
                            return True
            else:
