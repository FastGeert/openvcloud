...
class cloudspaces:
    ...
    def create_cloud_space(self, account, cloud_space=""):
        ...
        self.framework.cloud_space_name = (
            cloud_space or str(uuid.uuid4()).replace("-", "")[0:10]
        )
        ...
        self.framework.assertEqual(
            self.framework.get_text("create_cloud_space"), "Create Cloud Space"
        )
        ...
        self.framework.wait_until_element_located_and_has_text(
            "cloud_space_table_first_element_2", self.framework.cloud_space_name
        )
    ...
    def open_cloudspace_page(self, cloudspace=""):
        ...
        self.framework.wait_until_element_located_and_has_text(
            "cloud_space_table_first_element_2", cloudspace
        )
    ...
    def delete_cloudspace(self, cloudspace=""):
        ...
        for _ in range(50):
            if self.framework.get_text("cloudspace_page_status") in [
                "DEPLOYED",
                "VIRTUAL",
                "DESTROYED",
            ]:
        ...
        if self.framework.get_text("cloudspace_page_status") != "DESTROYED":
            ...
            for temp in range(10):
                if self.framework.wait_until_element_located_and_has_text(
                    "cloudspace_page_status", "DESTROYED"
                ):
                    return True
                else:
                    self.framework.get_page(self.framework.driver.current_url)
