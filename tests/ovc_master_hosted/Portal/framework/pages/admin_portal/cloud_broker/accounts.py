...
class accounts:
    ...
    def create_new_account(self, account="", username="", max_memory=None):
        ...
        self.framework.assertTrue(
            self.framework.check_element_is_exist("create_account")
        )
        ...
        self.framework.wait_until_element_located_and_has_text(
            "account_table_first_element", account
        )
    ...
    def account_edit(self, account, edit_item, edit_value):
        ...
        self.framework.assertEqual(
            self.framework.get_text("account_action_edit_page"), "Confirm Action Edit"
        )

        edit_items = [
            "account_edit_Name",
            "account_edit_Max Memory Capacity",
            "account_edit_Max VDisk Capacity",
            "account_edit_Max Number of CPU Cores",
            "account_edit_Max External Network Transfer",
            "account_edit_Max Number of Public IP Addresses",
        ]
    ...
    def account_edit_all_items(self, account):
        ...
        for item in edit_items:
            if item == "account_edit_Name":
                ...
                xpath = self.framework.elements["account_action_page_items"][
                    1
                ] % edit_items.index(item)
                try:
                    for _ in range(10):

                        if (
                            str(value)
                            in self.framework.driver.find_element_by_xpath(xpath).text
                        ):
                            break
                        else:
                            time.sleep(2)
                        ...
                        self.framework.lg(
                            ...
                            % (
                                value,
                                self.framework.driver.find_element_by_xpath(xpath).text,
                            )
                        )
                        return False
                except:
    ...
    def account_disable(self, account):
        ...
        if self.framework.get_text("account_page_status") != "CONFIRMED":
            self.framework.lg(
                "FAIL : %s account status : %s"
                % (account, self.framework.get_text("account_page_status"))
            )
            return False
        ...
        self.framework.assertEqual(
            self.framework.get_text("account_disable_page"), "Confirm Action Disable"
        )
        ...
        for _ in range(10):
            ...
            self.framework.lg(
                "FAIL : account status : %s"
                % self.framework.get_text("account_page_status")
            )
            return False
    ...
    def account_enable(self, account):
        ...
        time.sleep(2)
        self.framework.assertEqual(
            self.framework.get_text("account_enable_page"), "Confirm Action Enable"
        )
    ...
    def delete_account(self, account=""):
        ...
        if self.framework.driver.find_element_by_xpath(
            self.framework.elements["account_page_status"][1]
        ).text in ["CONFIRMED", "DISABLED"]:
            ...
            self.framework.fail(
                '"%s" account status has an error in the page' % account
            )
