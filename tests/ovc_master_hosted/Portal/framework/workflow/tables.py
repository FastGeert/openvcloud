...
class tables:
    ...
    def wait_until_table_reload(self, table, page, list_size=100):
        ...
        for _ in range(10):
            ...
            if (
                end_number == expected_end_number
                and start_number == expected_start_number
            ):
                return True
            else:
                time.sleep(1)
            ...
            self.framework.lg(
                "Error expected start & end numbers : %s , actual : %s"
                % (
                    str([expected_start_number, expected_end_number]),
                    str([start_number, end_number]),
                )
            )
            return False
    ...
    def get_table_start_number(self, table_info):
        ...
        return int(
            account_info[
                (account_info.index("g") + 2) : (account_info.index("to") - 1)
            ].replace(",", "")
        )
    ...
    def get_table_end_number(self, table_info):
        ...
        return int(
            account_info[
                (account_info.index("to") + 3) : (account_info.index("of") - 1)
            ].replace(",", "")
        )
    ...
    def get_table_max_number(self, table_info):
        ...
        return int(
            account_info[
                (account_info.index("f") + 2) : (account_info.index("entries") - 1)
            ].replace(",", "")
        )
    ...
    def check_sorting_table(self, table):
        ...
        for column, element in enumerate(table_head_elements):
            ...
            self.framework.driver.execute_script(
                "window.scrollTo(0,%d)" % (table_location["y"] - 50)
            )
            element.click()
            self.framework.wait_until_element_attribute_has_text(
                element, "aria-sort", "ascending"
            )
            table_before = self.get_table_data(table, column)

            if table_before == False:
                self.framework.lg(
                    "Error while collecting data from column %s after sorting "
                    % current_column
                )
                return False
            elif table_before == []:
            ...
            if len(table_before) != len(table_after):
                self.framework.lg(
                    (
                        "Length of collected data before sorting not equal to the "
            ...
            if table_before != list(reversed(table_after)):
                self.framework.lg(
                    "Sorting data in column %s does't work as expected" % current_column
                )
                return False
    ...
    def check_search_box(self, table, column_name):
        ...
        if not any(random_row[column_index] in s for s in first_row_after):
            self.framework.lg(
                "searching for value in calumn %s doesn't work as expected"
                % column_name
            )
            return False
        self.framework.clear_text(table["search_box"])
        if not self.framework.wait_until_element_located_and_has_text(
            table["info"], info_table_before
        ):
            self.framework.lg(
                "table doesn't update table before search %s and after %s"
                % (self.framework.get_text(table["info"]), info_table_before)
            )
            return False
        return True
