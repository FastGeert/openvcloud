...
class login:
    ...
    def Login(self, username="", password="", cookies_login=False, portal="admin"):
        if cookies_login:
            ...
            cookies = {
                "name": "beaker.session.id",
                "value": self.framework.beaker_session_id,
            }
            ...
            self.framework.assertEqual(
                self.framework.driver.title, "Log in - It's You Online"
            )
            ...
            if self.framework.find_elements("authentication_method"):
                if (
                    self.framework.get_text(element="authentication_method")
            ...
            for _ in range(25):
                if not self.framework.driver.title:
                    ...
                    self.framework.assertEqual(
                        self.framework.driver.title, title, "Can't Login"
                    )
