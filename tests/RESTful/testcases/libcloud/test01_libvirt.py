...
class LibCloudBasicTests(TestcasesBase):
    ...
    def test03_register_release_network_id_range(self, case, response_code):
        ...
        if case in ["valid_newtwork_id", "invalid_network_id"]:
            ...
            cloudspace_id = self.api.create_cloudspace(
                accountId=account_id, location=self.location
            )
            self.assertTrue(cloudspace_id)

            response = self.api.cloudbroker.cloudspace.getVFW(
            ...
            data, response = self.api.libcloud.libvirt.registerNetworkIdRange(
                gid=gid, start=start, end=end
            )
            self.assertEqual(response.status_code, response_code)
