...
class PermissionsTests(TestcasesBase):
    ...
    def test01_get_account(self, role, status_code):
        ...
        """
        self.log.info(
            "Try to get account (A1)'s info using user %s, should %s"
    ...
    def test02_update_account(self, role, status_code):
        ...
        """
        self.log.info(
            "Try to update account (A1)'s info using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
        api = self.admin_api if role == "admin" else self.user_api
    ...
    def test03_add_user(self, role, status_code):
        ...
        self.log.info(
            "Try to add user (U4) to account (A1) using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
        api = self.admin_api if role == "admin" else self.user_api
        response = api.cloudapi.accounts.addUser(
            accountId=self.account_id, userId=userId
        )
        self.assertEqual(response.status_code, status_code, response.content)
    ...
    def test04_update_user(self, role, status_code):
        ...
        self.log.info(
            "Try to update user (U4) access of account (A1) using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
        api = self.admin_api if role == "admin" else self.user_api
        data, response = api.cloudapi.accounts.updateUser(
            accountId=self.account_id, userId=userId
        )
        self.assertEqual(response.status_code, status_code, response.content)
    ...
    def test05_delete_user(self, role, status_code):
        ...
        self.log.info(
            "Try to delete user (U4) from account (A1) using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
        api = self.admin_api if role == "admin" else self.user_api
        response = api.cloudapi.accounts.deleteUser(
            accountId=self.account_id, userId=userId
        )
        self.assertEqual(response.status_code, status_code, response.content)
    ...
    def test06_get_consumed_cloud_units(self, role, status_code):
        ...
        """
        self.log.info(
            "Try to get consumed cloud units of account (A1) using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
        api = self.admin_api if role == "admin" else self.user_api
        response = api.cloudapi.accounts.getConsumedCloudUnits(
    ...
    def test07_get_consumed_cloud_units_by_type(self, role, status_code):
        ...
        """
        self.log.info(
            "Try to get consumed cloud units of account (A1) by type using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
        api = self.admin_api if role == "admin" else self.user_api
        response = api.cloudapi.accounts.getConsumedCloudUnitsByType(
            accountId=self.account_id
        )
        self.assertEqual(response.status_code, status_code, response.content)
    ...
    def test08_get_consumption(self, role, status_code):
        ...
        self.log.info(
            "Try to get consumption of account (A1) by type using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
        ...
        response = api.cloudapi.accounts.getConsumption(
            accountId=self.account_id, start=start, end=end
        )
        self.assertEqual(response.status_code, status_code, response.content)
    ...
    def test10_list_templates(self, role, status_code):
        ...
        """
        self.log.info(
            "Try to list account (A1)'s templates using %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
        api = self.admin_api if role == "admin" else self.user_api
