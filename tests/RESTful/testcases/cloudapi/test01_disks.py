...
class PermissionsTests(TestcasesBase):
    def setUp(self):
        ...
        self.disk_data, self.response = self.api.cloudapi.disks.create(
            accountId=self.account_id, gid=self.gid
        )
    ...
    def test01_create_disk(self, role, response_code):
        ...
        """
        self.log.info(
            "Try create disk on account (A1) using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )

        api = self.admin_api if role == "admin" else self.user_api
        data, response = api.cloudapi.disks.create(
            accountId=self.account_id, gid=self.gid
        )
    ...
    def test02_list_disks(self, role, response_code):
        ...
        """
        self.log.info(
            "Try to list account (A1)'s disks using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
    ...
    def test03_get_disk(self, role, response_code):
        ...
        """
        self.log.info(
            "Try to get disk (D1) using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
    ...
    def test04_resize_disk(self, role, response_code):
        ...
        """
        self.log.info(
            "Try to resize disk (D1) using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
    ...
    def test05_delete_disk(self, role, response_code):
        ...
        """
        self.log.info(
            "Try to delete disk (D1) using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
    ...
    def test06_limit_disk_io(self, role, response_code):
        ...
        """
        self.log.info(
            "Try to limit disk (D1) io using user %s, should %s"
            % (("(U2)", "succeed") if role == "admin" else ("(U3)", "fail"))
        )
...
class OperationTests(TestcasesBase):
    ...
    @parameterized.expand(
        [("exists_account", 200), ("non_exists_account", 404), ("invalid_account", 400)]
    )
    ...
    @parameterized.expand(
        [("exists_disk", 200), ("non_exists_disk", 404), ("invalid_disk", 400)]
    )
    ...
    @parameterized.expand(
        [("exists_disk", 200), ("non_exists_disk", 404), ("invalid_disk_size", 400)]
    )
