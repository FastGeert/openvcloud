...
class AgentController(TestcasesBase):
    ...
    def test001_swap_consuming(self):
        ...
        response = self.api.system.agentcontroller.executeJumpscript(
            gid=gid, cmd=cmd, nid=node_id
        )
        self.assertEqual(response.status_code, 200, response.content)
        time.sleep(30)
        ...
        self.api.system.agentcontroller.executeJumpscript(
            gid=gid, cmd=cmd, timeout=60, nid=node_id
        )
        ...
        swap_data = [
            data
            for data in node_status["System Load"]["data"]
            if "Swap used value is" in data["msg"]
        ][0]
