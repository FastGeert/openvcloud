...
class permission(TestcasesBase):
    def setUp(self):
        ...
        self.user_api.system.usermanager.authenticate(
        ...
        self.cloudspace, response = self.api.cloudbroker.cloudspace.create(
            accountId=self.accountId, location=self.location, access=self.user
        )
        self.assertEqual(response.status_code, 200)
        self.cloudspaceId = response.json()
        self.assertEqual(
            self.api.wait_for_cloudspace_status(self.cloudspaceId), "DEPLOYED"
        )
    ...
    def test001_adduser_with_diff_users_access(self, access, return_code):
        ...
        **Test Scenario:**
        ...
        self.log.info(
            "Add user to the cloudspace with %s level user, should %s  "
            % (access, "succeed" if access == "admin" else "fail")
        )
        accesstype = random.choice(["R", "RCX", "ARCXDU"])
        data, response = api.cloudbroker.cloudspace.addUser(
    ...
    def test002_delete_user_with_diff_users_access(self, access, return_code):
        ...
        **Test Scenario:**
        ...
        self.log.info(
            "delete user from the cloudspace with %s level user, should %s  "
            % (access, "succeed" if access == "admin" else "fail")
        )
        response = api.cloudbroker.cloudspace.deleteUser(
            cloudspaceId=self.cloudspaceId, username=self.user
        )
        self.assertEqual(response.status_code, return_code)
    ...
    def test003_create_cloudspace_with_diff_users_access(self, access, return_code):
        ...
        **Test Scenario:*
        ...
        self.log.info(
            "Create cloudspace with %s level user, should %s  "
            % (access, "succeed" if access == "admin" else "fail")
        )
        data, response = api.cloudbroker.cloudspace.create(
            accountId=self.accountId, location=self.location, access=self.user
        )
        self.assertEqual(response.status_code, return_code)

    @parameterized.expand([("user", 403), ("admin", 200)])
    def test004_destroy_cloudspace_with_different_users_access(
        self, access, return_code
    ):
        """ OVC-000
        ...
        **Test Scenario:*
        ...
        self.log.info(
            "Destroy cloudspace with %s level user, should %s  "
            % (access, "succeed" if access == "admin" else "fail")
        )
        response = api.cloudbroker.cloudspace.destroy(
            accountId=self.accountId, cloudspaceId=self.cloudspaceId
        )
        self.assertEqual(response.status_code, return_code)
    ...
    def test005_start_stop_fireWall(self, access, return_code):
        ...
        **Test Scenario:**
        ...
        self.log.info(
            "Deploy VFW with %s level user, should %s  "
            % (access, "succeed" if access == "admin" else "fail")
        )
        ...
        self.log.info(
            "Stop VFW with %s level user, should %s  "
            % (access, "succeed" if access == "admin" else "fail")
        )
        ...
        self.log.info(
            "Start VFW with %s level user, should %s  "
            % (access, "succeed" if access == "admin" else "fail")
        )
    ...
    def test006_destroy_cloudspaces(self, access, return_code):
        ...
        **Test Scenario:**
        ...
        self.log.info(
            " Destroy  cloudspaces with %s level user, should %s  "
            % (access, "succeed" if access == "admin" else "fail")
        )
        cloudspaces = [self.cloudspaceId]
        data, response = self.api.cloudbroker.cloudspace.create(
            accountId=self.accountId, location=self.location, access=self.user
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = api.cloudbroker.cloudspace.destroyCloudSpaces(
            [cloudspaces[1], cloudspaces[0]]
        )
        self.assertEqual(response.status_code, return_code)
    ...
    @parameterized.expand([("user", 403), ("admin", 200)])
    def test009_update_cloudspace_with_different_users_access(
        self, access, return_code
    ):
        """ OVC-000
        ...
        **Test Scenario:*
        ...
        data, response = api.cloudbroker.cloudspace.update(
            cloudspaceId=self.cloudspaceId
        )
        self.assertEqual(response.status_code, return_code)
    ...
    def test010_moveVirtualFirewallToFirewallNode(self, access, return_code):
        ...
        **Test Scenario:*
        ...
        reouteros_nid = self.api.cloudbroker.cloudspace.getVFW(
            self.cloudspaceId
        ).json()["nid"]
        targetNid = self.api.get_running_nodeId(reouteros_nid)
        self.log.info(
            "MoveVirtualFirewallToFirewallNode with %s level user, should %s  "
            % (access, "succeed" if access == "admin" else "fail")
        )
        response = api.cloudbroker.cloudspace.moveVirtualFirewallToFirewallNode(
            self.cloudspaceId, targetNid
        )
        self.assertEqual(response.status_code, return_code)
    ...
    def test011_update_cloudspace(self, access, return_code):
        ...
        **Test Scenario:*
        ...
        self.log.info(
            "update with %s level user, should %s  "
            % (access, "succeed" if access == "admin" else "fail")
        )
...
class operations(TestcasesBase):
    ...
    def test001_add_nonexistuser_to_cloudspace(self, status):
        ...
        **Test Scenario:**
        ...
        data, response = self.api.cloudbroker.cloudspace.addUser(
            username=user, cloudspaceId=self.cloudspaceId, accesstype="R"
        )
        self.assertEqual(response.status_code, 404)

    @parameterized.expand(
        [("R", 200, 403, 403), ("RCX", 200, 403, 200), ("ARCXDU", 200, 200, 200)]
    )
    @unittest.skip("https://github.com/0-complexity/openvcloud/issues/1436")
    def test002_add_user_to_cloudspace(
        self, accesstype, get_code, update_code, vm_code
    ):
        """ OVC-000
        ...
        **Test Scenario:**
        ...
        self.log.info(
            "Add user[U1] to [CS1] with access[%s], should succeed." % accesstype
        )
        data, response = self.api.cloudbroker.cloudspace.addUser(
            username=user_data["username"],
            cloudspaceId=self.cloudspaceId,
        ...
        data, response = self.user_api.cloudapi.cloudspaces.update(
            cloudspaceId=self.cloudspaceId
        )
        self.assertEqual(response.status_code, update_code)

    @parameterized.expand([("non-exist", 404), ("exist", 200), ("deleted", 404)])
    def test003_start_stop_fireWall(self, value, return_code):
        ...
        **Test Scenario:**
        ...
        if value == "non-exist":
            ...
            response = self.api.cloudbroker.cloudspace.destroy(
                self.accountId, self.cloudspaceId
            )
            cloudspaceId = self.cloudspaceId
        else:
            cloudspaceId = self.cloudspaceId
        ...
        self.log.info(
            "Deploy VFW to %s cloudspace , should %s"
            % (value, "fail" if value != "exist" else "succeed")
        )
        response = self.api.cloudbroker.cloudspace.deployVFW(cloudspaceId)
        self.assertEqual(response.status_code, return_code)

        self.log.info(
            "Stop VFW to %s cloudspace , should %s"
            % (value, "fail" if value != "exist" else "succeed")
        )
    ...
    @parameterized.expand([("Negative values", -1, 400), ("Positive values", 1, 200)])
    def test004_create_cloudspace_with_different_options(
        self, type, factor, return_code
    ):
        """ OVC-000
        ...
        **Test Scenario:**
        ...
        self.log.info(
            "Create account with passing %s values in the cloudspace's limitation."
            % type
        )
        cloudspace_limitation = {
            "maxMemoryCapacity": random.randint(2, 1000) * factor,
    ...
    def test005_create_cloudspace_with_limitations(self):
        ...
        **Test Scenario:**
        ...
        account_limitation = {
            "maxMemoryCapacity": random.randint(2, 1000),
            "maxVDiskCapacity": random.randint(2, 1000),
            "maxNetworkPeerTransfer": random.randint(2, 1000),
            "maxNumPublicIP": 1,
        }
        data, account = self.api.cloudbroker.account.create(
            username=self.user, **account_limitation
        )
        ...
        cloudspace_limitation = {
            cloudspacelimit: account_limitation[cloudspacelimit] + 1
        }
        data, response = self.api.cloudbroker.cloudspace.create(
        ...
        data, response = self.api.cloudbroker.cloudspace.create(
            accountId=account.json(), location=self.location, access=self.user
        )
        self.assertEqual(response.status_code, 200)
    ...
    @parameterized.expand([("exist", 200), ("Non-exist", 404)])
    def test006_destroy_cloudspace(self, status, return_code):
        ...
        **Test Scenario:**
        ...
        self.log.info(
            "Destroy  %s cloudspace, should %s"
            % (status, "succeed" if status == "exist" else "fail")
        )
    ...
    def test007_destroy_cloudspaces(self):
        ...
        **Test Scenario:**
        ...
        for _ in range(3):
            data, response = self.api.cloudbroker.cloudspace.create(
                self.accountId, self.location, self.user
            )
            self.assertEqual(response.status_code, 200)
            cloudspaces.append(response.json())
        ...
        response = self.api.cloudbroker.cloudspace.destroy(
            self.accountId, cloudspaces[0]
        )
        self.assertEqual(response.status_code, 200)

        self.log.info(" Delete cloudspaces [CS1], [CS2], should fail.")
        response = self.api.cloudbroker.cloudspace.destroyCloudSpaces(
            [cloudspaces[1], cloudspaces[0]]
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test009_delete_non_exist_user_from_cloudspace(self):
        ...
        **Test Scenario:**
        ...
        response = self.api.cloudbroker.cloudspace.deleteUser(
            cloudspaceId=self.cloudspaceId, username=fake_user
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test010_delete_user_from_cloudspace(self):
        ...
        **Test Scenario:**
        ...
        fake_user = self.utils.random_string()
        response = self.user_api.cloudbroker.cloudspace.deleteUser(
            cloudspaceId=self.cloudspaceId, username=fake_user
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test011_create_cloudspace_with_nonexist_user(self):
        ...
        **Test Scenario:*
        ...
        fake_user = self.utils.random_string()
        data, response = self.api.cloudbroker.cloudspace.create(
            accountId=self.accountId, location=self.location, access=fake_user
        )
        self.assertEqual(response.status_code, 404)

    @parameterized.expand(
        [
            ("non-exist-cloudspace", 404),
            ("Non-exist-node", 404),
    ...
    def test008_moveVirtualFirewallToFirewallNode(self, status, return_code):
        ...
        **Test Scenario:**
        ...
        """
        routeros_nid = self.api.cloudbroker.cloudspace.getVFW(self.cloudspaceId).json()[
            "nid"
        ]
        ...
        self.log.info(
            "MoveVirtualFirewallToFirewallNode with %s , should %s  "
            % (status, "succeed" if status == "exist-cloudspace" else "fail")
        )
        response = self.api.cloudbroker.cloudspace.moveVirtualFirewallToFirewallNode(
            cloudspaceId, targetNid
        )
        self.assertEqual(response.status_code, return_code)

        if status == "exist-cloudspace&node":
            self.log.info("Check that firewall moved successfully.")
            new_nid = self.api.cloudbroker.cloudspace.getVFW(self.cloudspaceId).json()[
                "nid"
            ]
            self.assertEqual(new_nid, targetNid)

    @parameterized.expand([("non-exist", 404), ("deleted", 404), ("exist", 200)])
