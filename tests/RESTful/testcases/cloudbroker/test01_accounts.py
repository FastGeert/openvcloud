...
class Test(TestcasesBase):
    ...
    def test001_create_account_with_different_options(self, type, factor, return_code):
        ...
        **Test Scenario:**
        ...
        """
        self.log.info(
            "Create account with passing %s values in the account's limitation." % type
        )
        accounts_limitation = {
        ...
        data, response = self.api.cloudbroker.account.create(
            username=self.user, **accounts_limitation
        )
        self.assertEqual(
    ...
    def test002_create_account_with_non_existing_user(self):
        ...
        **Test Scenario:**
        ...
        self.assertEqual(
            response.status_code, 400, "Email address is required for new users."
        )

    @parameterized.expand(
        [("R", 200, 403, 403), ("RCX", 200, 403, 200), ("ARCXDU", 200, 200, 200)]
    )
    def test003_add_user_to_account(
        self, accesstype, get_code, update_code, cloudspace_code
    ):
        """ OVC-000
        ...
        **Test Scenario:**
        ...
        self.log.info(
            "Create account[C1] for main user and get this account  with  user,should succeed."
        )
        ...
        data, response = self.api.cloudbroker.account.addUser(
            username=user_data["username"],
            accountId=self.response.json(),
        ...
        data, response = self.user_api.cloudapi.cloudspaces.create(
            accountId=self.response.json(),
            location=self.location,
            access=user_data["username"],
        )
        self.assertEqual(response.status_code, cloudspace_code)

        new_account_name = self.utils.random_string()
        response = self.user_api.cloudapi.accounts.update(
            accountId=self.response.json(), name=new_account_name
        )
    ...
    def test005_delete_accounts(self):
        ...
        **Test Scenario:**
        ...
        response = self.api.cloudbroker.account.deleteAccounts(
    ...
    def test010_account_disable_and_enable(self):
        ...
        **Test Scenario:**
        ...
        data, response = self.api.cloudbroker.account.addUser(
            username=user_data["username"], accountId=c1_id, accesstype="ARCXDU"
        )
        self.assertEqual(response.status_code, 200)
        ...
        data, response = self.user_api.cloudapi.cloudspaces.create(
            accountId=c1_id, location=self.location, access=user_data["username"]
        )
        self.assertEqual(
            response.status_code,
            403,
            "Only READ actions can be executed on account (or one of its cloudspace or machines) with status DISABLED.",
        )
        ...
        data, response = self.user_api.cloudapi.cloudspaces.create(
            accountId=c1_id, location=self.location, access=self.user
        )
        self.assertEqual(response.status_code, 200)
