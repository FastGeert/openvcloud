...
class Client:
    ...
    def create_account(self, **kwargs):
        data, response = self.cloudbroker.account.create(
            username=self._whoami, **kwargs
        )
    ...
    def create_cloudspace(self, accountId, location, **kwargs):
        data, response = self.cloudapi.cloudspaces.create(
            accountId=accountId, location=location, access=self._whoami, **kwargs
        )
    ...
    def wait_for_cloudspace_status(self, cloudspaceId, status="DEPLOYED", timeout=300):
        ...
        for _ in range(timeout):
            if cs_status == status:
                break
            time.sleep(1)
            response = self.api_client.cloudapi.cloudspaces.get(
    ...
    def get_running_nodeId(self, except_nodeid=None):
        ...
        for node in nodes:
            if (
                int(node["referenceId"]) != except_nodeid
