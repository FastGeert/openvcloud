...
class Machine:
    ...
    def deleteDisk(self, machineId, diskId):
        return self._api.cloudbroker.machine.deleteDisk(
            machineId=machineId, diskId=diskId
        )

    def deleteUser(self, machineId, userId):
        return self._api.cloudbroker.machine.deleteUser(
            machineId=machineId, userId=userId
        )

    def deleteSnapshot(self, machineId, epoch, reason):
        return self._api.cloudbroker.machine.deleteSnapshot(
            machineId=machineId, epoch=epoch, reason=reason
        )
    ...
    def destroyMachines(self, machineIds, **kwargs):
        reason = kwargs.get("reason", utils.random_string())
        return self._api.cloudbroker.machine.destroyMachines(
            machineIds=machineIds, reason=reason
        )
    ...
    def listPortForwards(self, machineId, **kwargs):
        ...
        return self._api.cloudbroker.machine.listPortForwards(
            machineId=machineId, result=result
        )
    ...
    def rebootMachines(self, machineIds, **kwargs):
        reason = kwargs.get("reason", utils.random_string())
        return self._api.cloudbroker.machine.rebootMachines(
            machineIds=machineIds, reason=reason
        )
    ...
    def startMachines(self, machineIds, **kwargs):
        reason = kwargs.get("reason", utils.random_string())
        return self._api.cloudbroker.machine.startMachines(
            machineIds=machineIds, reason=reason
        )
    ...
    def stopMachines(self, machineIds, **kwargs):
        reason = kwargs.get("reason", utils.random_string())
        return self._api.cloudbroker.machine.stopMachines(
            machineIds=machineIds, reason=reason
        )
    ...
    def rollbackSnapshot(self, machineId, **kwargs):
        return self._api.cloudbroker.machine.rollbackSnapshot(
            machineId=machineId, **kwargs
        )
    ...
    def tag(self, machineId, **kwargs):
        data = {"machineId": machineId, "tag": utils.random_string()}
        data.update(**kwargs)
        return (
            data,
            self._api.cloudbroker.machine.tag(machineId=machineId, tagName=tagName),
        )
