...
class Users:
    ...
    def authenticate(self, username, password):
        return self._api.cloudapi.users.authenticate(
            username=username, password=password
        )

    def getMatchingUsernames(self, usernameregex, limit):
        return self._api.cloudapi.users.getMatchingUsernames(
            usernameregex=usernameregex, limit=limit
        )

    def getResetPasswordInformation(self, resettoken):
        return self._api.cloudapi.users.getResetPasswordInformation(
            resettoken=resettoken
        )

    def isValidInviteUserToken(self, inviteusertoken, emailaddress):
        return self._api.cloudapi.users.authenticate(
            inviteusertoken=inviteusertoken, emailaddress=emailaddress
        )

    def registerInvitedUser(
        self, inviteusertoken, emailaddress, username, password, confirmpassword
    ):
    ...
    def resetPassword(self, resettoken, newpassword):
        return self._api.cloudapi.users.resetPassword(
    ...
    def updatePassword(self, data):
        return self._api.cloudapi.users.updatePassword(
            oldPassword=oldPassword, newPassword=newPassword
        )

    def validate(self, validationtoken, password):
        return self._api.cloudapi.users.validate(
            validationtoken=validationtoken, password=password
        )
