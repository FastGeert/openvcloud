...
class Cloudspaces:
    ...
    def enable(self, cloudspaceId, **kwargs):
        ...
        return self._api.cloudapi.cloudspaces.enable(
            cloudspaceId=cloudspaceId, reason=reason
        )

    def disable(self, cloudspaceId, **kwargs):
        reason = kwargs.get("reason", utils.random_string())
        return self._api.cloudapi.cloudspaces.disable(
            cloudspaceId=cloudspaceId, reason=reason
        )

    def getDefenseShield(self, cloudspaceId):
        return self._api.cloudapi.cloudspaces.getDefenseShield(
            cloudspaceId=cloudspaceId
        )

    def getOpenvpnConfig(self, cloudspaceId):
        return self._api.cloudapi.cloudspaces.getOpenvpnConfig(
            cloudspaceId=cloudspaceId
        )

    def addAllowedSize(self, cloudspaceId, sizeId):
        return self._api.cloudapi.cloudspaces.addAllowedSize(
            cloudspaceId=cloudspaceId, sizeId=sizeId
        )

    def removeAllowedSize(self, cloudspaceId, sizeId):
        return self._api.cloudapi.cloudspaces.removeAllowedSize(
            cloudspaceId=cloudspaceId, sizeId=sizeId
        )

    def addUser(self, cloudspaceId, userId, accesstype="ARCXDU"):
        return self._api.cloudapi.cloudspaces.addUser(
            cloudspaceId=cloudspaceId, userId=userId, accesstype=accesstype
        )
    ...
    def executeRouterOSScript(self, cloudspaceId, script):
        return self._api.cloudapi.cloudspaces.executeRouterOSScript(
            cloudspaceId=cloudspaceId, script=script
        )
