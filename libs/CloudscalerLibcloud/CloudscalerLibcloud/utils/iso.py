...
class ISO(object):
    def generate_user_yaml(self, user_data, fp):
        ...
        yaml.safe_dump(stream=fp, data=user_data, default_flow_style=False)
        fp.flush()

    def generate_meta_yaml(self, meta_data, fp):
        fp.write("#cloud-config\n")
        yaml.safe_dump(stream=fp, data=meta_data, default_flow_style=False)
        fp.flush()
    ...
    def generate_meta_iso(self, fp, meta_data, user_data):
        def gentemp(prefix):
            return tempfile.NamedTemporaryFile(
                prefix="cloudscalers.{prefix}.".format(prefix=prefix), suffix=".tmp"
            )
    ...
    def create_meta_iso(self, output, meta_data, user_data, type, volume_exists=False):
        with tempfile.TemporaryFile() as iso:
            if type not in ["WINDOWS", "Windows", "windows"]:
                self.generate_meta_iso(fp=iso, meta_data=meta_data, user_data=user_data)
            else:
                self.generate_windows_meta_iso(
                    fp=iso, meta_data=meta_data, user_data=user_data
                )
            iso.seek(0)
            ...
            proc = subprocess.Popen(
                cmd, stdin=iso, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            proc.wait()
            if proc.returncode != 0:
                raise RuntimeError(
                    "Failed to create metadata iso. Error: %s / %s"
