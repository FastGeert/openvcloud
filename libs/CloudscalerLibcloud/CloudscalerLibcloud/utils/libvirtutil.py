...
class LibvirtUtil(object):
    ...
    def modXML(self, xml):
        ...
        vcpu.set(
            "cpuset",
            "{startcpu}-{cpulimit}".format(
                startcpu=RESERVED_CPUS, cpulimit=CPU_COUNT - 1
            ),
        )
        ...
        if domain:
            ...
            if domain.state()[0] not in [
                ...
                if force:
                    ...
                    try:
                        self.waitForAction(
                            id, timeout=30000, events=[libvirt.VIR_DOMAIN_EVENT_STOPPED]
                        )
        ...
        for disk in self.get_domain_disks(dom):
            ...
            if source is not None:
                if source.attrib.get("protocol") == "openvstorage":
                    diskfiles.append(
                        os.path.join(self.basepath, source.attrib["name"] + ".raw")
                    )
                else:
    ...
    def snapshot(self, id, xml, snapshottype):
        ...
        flags = (
            0
            if snapshottype == "internal"
            else libvirt.VIR_DOMAIN_SNAPSHOT_CREATE_DISK_ONLY
        )
    ...
    def _clone(self, id, filename, clonefrom):
        ...
        if domain.state()[0] in [
            ...
            size = int(
                j.system.platform.qemu_img.info(clonefrom, unit="")["virtual size"]
            )
            ...
            j.system.platform.qemu_img.convert(
                clonefrom, "raw", destination_path, "raw", createTarget=False
            )
        else:
            domain.undefine()
            try:
