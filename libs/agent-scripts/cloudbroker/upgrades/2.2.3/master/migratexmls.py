...
def action():
    ...
    machines = ccl.vmachine.search(
        {"status": {"$nin": ["ERROR", "DESTROYED"]}}, size=0
    )[1:]
    for machine in machines:
        ...
        vmdisks = [
            ...
            for disk in ccl.disk.search({"id": {"$in": machine["disks"]}})[1:]
        ]
        ...
        for xmldisk in xmldisks:
            ...
            if not diskpair:
                ...
                disk.referenceId = (
                    firstdisk["referenceId"]
                    .replace(firstvolume.vdiskguid, diskguid)
                    .replace(firstvolume.name, name)
                )
