...
def action(url, volumeid, eventstreamid, disksize, ovs_connection, istemplate=False):
    ...
    def resize_disk(newsize):
        diskextend = j.clients.redisworker.getJumpscriptFromName(
            "greenitglobe", "extend_disk"
        )
        diskextend.write()
        diskextend.load()
    ...
    def reporthook(percent):
        ...
        pcl.actors.system.contentmanager.sendEvent(
            title, message.format(percent), level, eventstreamid
        )
    ...
    try:
        ...
        pcl.actors.system.contentmanager.sendEvent(
            title, message.format(100), level, eventstreamid
        )
