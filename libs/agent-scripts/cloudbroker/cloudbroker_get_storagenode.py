...
def action(interface="storage"):
    ...
    storage_router = ovscl.get(
        "/storagerouters",
        params=dict(
            contents="node_type,vpools_guids,storagedrivers,vdisks_guids",
            query=json.dumps(storage_router_query),
        ),
    )["data"][0]
    ...
    vpools = ovscl.get(
        "/vpools",
        params=dict(contents="name,_relations", query=json.dumps(vpools_query)),
    )["data"]
    ...
    storage_drivers = ovscl.get(
        "/storagedrivers",
        params=dict(contents="name,guid,ports", query=json.dumps(storage_driver_query)),
    )["data"]

    for vpool in vpools:
        for storage_driver in storage_drivers:
            if storage_driver["guid"] in vpool["storagedrivers_guids"]:
                node_data["storage_drivers"][vpool["name"]] = {
                    "ports": storage_driver["ports"]
                }
    return node_data
