...
def action(name, vxlanid, pubips, dmzips):
    ...
    j.system.platform.lxc.networkSetPublic(
        name, netname="pub0", bridge=bridge, pubips=pubips, gateway=gateway
    )
    j.system.platform.lxc.networkSetPrivateOnBridge(
        name, netname="dmz0", bridge=bridge, ipaddresses=dmzips
    )
