...
def action(
    ovs_connection, storagerouterguid, diskname, size, templateguid, pagecache_ratio
):
    ...
    free = sum(
        [
            ...
            for stat in statsclient.statsByPrefix(
                "ovs.backend.free@{backend_name}".format(backend_name=backend_name)
            )
        ]
    )
    used = sum(
        [
            stat.val
            for stat in statsclient.statsByPrefix(
                "ovs.backend.used@{backend_name}".format(backend_name=backend_name)
            )
        ]
    )
    ...
    if total and (used * 100.0 / total) >= 80:
        raise Exception(
            "Used capacity on {backend_name} >= 80%".format(backend_name=backend_name)
        )
...
if __name__ == "__main__":
    ...
    action(
        ovs,
        options.storagerouterguid,
        options.diskname,
        options.size,
        options.templateguid,
        20,
    )
