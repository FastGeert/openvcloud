...
def get_cached_accounts():
    ...
    accounts_ids = map(
        get_ids,
        accl.search(
            {"$query": {"status": {"$ne": "DESTROYED"}}, "$fields": ["id"]}, size=0
        )[1:],
    )
    cloudspaces = cscl.search(
        {
            "$query": {
                "accountId": {"$in": accounts_ids},
                "status": {"$ne": "DESTROYED"},
                "gid": j.application.whoAmI.gid,
            },
            "$fields": ["id", "accountId", "status", "gid", "networkId"],
        },
        size=0,
    )[1:]
    vms = vmcl.search(
        {
            "$query": {
                "cloudspaceId": {"$in": map(get_ids, cloudspaces)},
                "status": {"$ne": "DESTROYED"},
            },
            "$fields": [
                "id",
    ...
    disks = {
        disk["id"]: disk
        for disk in dcl.search(
            {"$query": {"id": {"$in": diskids}}, "$fields": ["id", "sizeMax"]}, size=0
        )[1:]
    }
    ...
    for accountid, spaces in spacesperaccount.iteritems():
        for space in spaces:
            space = cached_accounts.setdefault(accountid, {}).setdefault(
                space["id"], space
            )
            space["vms"] = []
...
def get_val(redis, key):
    ...
    if value:
        ...
        if value.get("h_last_epoch", 0):
            if (
                now - datetime.utcfromtimestamp(float(value["h_last_epoch"]))
        ...
        for cloudspace_id, cs in cloudspaces_dict.items():
            ...
            if cs["status"] == "DEPLOYED" and cs["networkId"] in virtualfirewalls:
                ...
                publicTX = get_last_hour_val(
                    redis,
                    "stats:{gid}_{nid}:network.vfw.packets.rx@virt.pub-{id}".format(
                        **data
                    ),
                )
                publicRX = get_last_hour_val(
                    redis,
                    "stats:{gid}_{nid}:network.vfw.packets.tx@virt.pub-{id}".format(
                        **data
                    ),
                )
                spaceRX = get_last_hour_val(
                    redis,
                    "stats:{gid}_{nid}:network.vfw.packets.rx@virt.spc-{id}".format(
                        **data
                    ),
                )
                spaceTX = get_last_hour_val(
                    redis,
                    "stats:{gid}_{nid}:network.vfw.packets.tx@virt.spc-{id}".format(
                        **data
                    ),
                )
            else:
