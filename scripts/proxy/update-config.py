...
if len(refsrv) == 0:
    ...
    for service in j.atyourservice.findServices(name="node.ssh"):
        ...
        if ovsservice or cpuservice:
            ...
            if ovsservice:
                ...
                upstreams.setdefault("ovs-" + envname, []).append(
                    "%s:%s" % (host, port)
                )
            if cpuservice:
    ...
    for service in j.atyourservice.findServices(
        name="autossh", instance="http_proxy_ovs"
    ):
        location = service.parent.parent.instance
        host = service.hrd.getStr("instance.remote.bind")
        port = service.hrd.getStr("instance.remote.port")
...
proxy.execute(
    "rm -f /opt/jumpscale7/hrd/apps/openvcloud__ssloffloader__main/installed.version"
)
...
masteraioconfig = j.application.getAppInstanceHRD(
    name="cb_master_aio", instance="main", domain="openvcloud"
)
...
master.execute(
    "rm -f /opt/jumpscale7/hrd/apps/jumpscale__portal__main/installed.version"
)
