...
def main(ipaddress):
    ...
    query = {
        "$query": {
            "referenceId": {"$regex": ipaddress.replace(".", "\.")},
            "status": {"$ne": "DESTROYED"},
        }
    }
    ...
    print(diskids)
    query = {"$query": {"disks": {"$in": diskids}, "status": {"$ne": "DESTROYED"}}}
    ...
    for idx, vm in enumerate(vms):
        ...
        if vmdata["status"] == "RUNNING":
            print(
                "Stopping vm {name} {nr}/{count}".format(
                    name=vmdata["name"], nr=idx + 1, count=len(vms)
                )
            )
            pcl.actors.cloudapi.machines.stop(vm["id"])
            runningvms.append(vm["id"])

    storagedrivers = scl.node.search(
        {"netaddr.ip": {"$ne": ipaddress}, "roles": "storagedriver"}
    )[1:]
    storagedriverips = []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--ipaddress", help="IP Address of the failted OVS node", required=True
    )
