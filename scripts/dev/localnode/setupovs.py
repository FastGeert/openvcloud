...
def get_loop_devices():
    ...
    for device in json.loads(output)["loopdevices"]:
        if hostname in device["back-file"]:
            devices[
                os.path.basename(device["back-file"]).split("-")[1]
            ] = os.path.basename(device["name"])
    return devices
...
diskinfo = ovscl.get(
    "/disks", params={"storagerouterguid": storagerouterguid, "contents": "_relations"}
)
...
addrolesdata = {
    ...
    "offset": 1048576,
    "disk_guid": disk["guid"],
    "partition_guid": disk["partitions_guids"][0],
    "roles": ["DB", "DTL", "SCRUB", "WRITE"],
}
ovscl.post(
    "/storagerouters/{}/configure_disk".format(storagerouterguid),
    data=json.dumps(addrolesdata),
)
...
albabackendguid = ""
albabackendguid = ovscl.post("/alba/backends/", data=json.dumps(albabackenddata))[
    "guid"
]
time.sleep(5)
...
while True:
    info = ovscl.get(
        "/alba/backends/{}/".format(albabackendguid),
        params={"contents": "_dynamics,_relations"},
    )
...
ovscl.post(
    "/alba/backends/{}/update_preset/".format(albabackendguid),
    data=json.dumps(presetdata),
)
...
taskguid = ovscl.post(
    "/alba/nodes/{}/initialize_disks/".format(nodeguid),
    data=json.dumps(initializediskdata),
)
...
def get_asd_info():
    backenddata = ovscl.get(
        "/alba/backends/{}/".format(albabackendguid),
        params={"contents": "_dynamics,-statistics.-ns_data._relations"},
    )["local_stack"]
...
print("Claiming ASD")
taskguid = ovscl.post(
    "/alba/backends/{}/add_units/".format(albabackendguid),
    data=json.dumps(claimasddata),
)
...
createpooldata = {
    "call_parameters": {
        ...
        "backend_info": {"preset": "default", "alba_backend_guid": albabackendguid},
        "storage_ip": "172.17.1.10",
        "storagerouter_ip": "172.17.1.10",
        "writecache_size": 3,
...
taskguid = ovscl.post(
    "/storagerouters/{}/add_vpool/".format(storagerouterguid),
    data=json.dumps(createpooldata),
)
