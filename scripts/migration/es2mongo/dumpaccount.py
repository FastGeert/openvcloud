...
def do(username):
    ...
    for cloudspace in ccl.cloudspace.simpleSearch({"accountId": accountId}):
        if (
            ...
            and cloudspace["location"] == locationname
        ):
    ...
    for cloudspace in data["cloudspaces"]:
        for vmachine in ccl.vmachine.simpleSearch({"cloudspaceId": cloudspace["id"]}):
            if vmachine["status"] != "DESTROYED" and vmachine["referenceId"]:
                vmachine["xml"] = lcl.libvirtdomain.get(
                    "domain_%(referenceId)s" % vmachine
                )
...
if __name__ == "__main__":
    ...
    if opts.all:
        ...
        for account in ccl.account.simpleSearch({"status": "CONFIRMED"}):
            for space in ccl.cloudspace.simpleSearch(
                {"status": "DEPLOYED", "accountId": account["id"]}
            ):
                machines = ccl.vmachine.simpleSearch(
                    {"cloudspaceId": space["id"], "status": "RUNNING"}
                )
                if machines:
