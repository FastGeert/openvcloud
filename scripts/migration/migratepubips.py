...
def migrate(network, gateway):
    ...
    allips = set(
        j.application.config.getDict("cloudscalers.networks.public_ip").values()
    )
    ...
    for spaced in cloudspaces:
        if spaced["status"] != "DESTROYED":
            ...
            if "/" not in space.publicipaddress and space.publicipaddress:
                space.publicipaddress = "%s/%s" % (
                    space.publicipaddress,
                    network.prefixlen,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--network",
        required=True,
        help="Network including cidr eg. 10.101.190.0/24",
    )
