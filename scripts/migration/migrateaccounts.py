...
IMAGEMAP = {"New Windows 2012r2 Standard": "Windows 2012r2 Standard"}
...
class ProcessPool(object):
    ...
    def run(self):
        ...
        while self.queuedjobs or [x for x in self.activejobs if x]:
            ...
            if self.queuedjobs:
                ...
                for emptyslot in emptyslots:
                    ...
                    process = multiprocessing.Process(
                        target=work[0], args=work[2], kwargs=work[3]
                    )
                    process.start()
            ...
            statusline = "Queued/Running/Finished/Total {}/{}/{}/{} Jobs ({}s)".format(
                queuedjobs, runningjobs, self.finishedjobs, totaljobs, refreshtime
            )
            if remainingtime < 0:
                messages = [statusline]
...
class Migrator(object):
    def __init__(
        self, debug=False, dryrun=False, concurrency=1, cloudspaces=None, vms=None
    ):
        ...
        self.ovscl = j.clients.openvstorage.get(
    ...
    def migrate_account(self, accountId):
        ...
        accounts = self.ccl.account.search(
            {"name": account.name, "status": {"$ne": "DESTROYED"}}
        )[1:]
        if not accounts:
        ...
        for cloudspace in self.source_ccl.cloudspace.search(
            {"accountId": accountId, "status": "DEPLOYED"}
        )[1:]:
            if (
                self.cloudspaces is not None
                and cloudspace["id"] not in self.cloudspaces
            ):
                continue
    ...
    def get_vms(self, cs_id):
        ...
        def vmfilter(vm):
            if self.vms and vm["id"] not in self.vms:
                return False
            return (
                self.source_ccl.disk.count({"id": {"$in": vm["disks"]}, "type": "P"})
                == 0
            )
    ...
    def migrate_space(self, cloudspace, newaccount):
        ...
        vfwdata = self.source_vfw.virtualfirewall.searchOne(
            {"id": cloudspacedata["networkId"]}
        )
        sourceip = self.get_source_ip(vfwdata["nid"])
        newcloudspaceId = self.pcl.actors.cloudbroker.cloudspace.migrateCloudspace(
            newaccount.id, cloudspacedata, vfwdata, sourceip, self.rgid
        )
    ...
    def vm_status(self, vmid, cloudspace, *args):
        try:
            ...
            return "VM {} is being migrated disk at {:.2f}%".format(
                vmid, (usedsize / totalsize) * 100
            )
        ...
        for disk in srcdom.findall("devices/disk"):
            ...
            if "username" in source.attrib:
                ...
                if "username" in newsource.attrib:
                    source.attrib[
                        "name"
                    ] += ":username={username}:password={passwd}".format(
                        **newsource.attrib
                    )
    ...
    def migrate_vm(self, vmid, newspace):
        ...
        if self.dryrun:
            data = {
                "xml": "xml",
                "stackId": self.ccl.stack.list()[0],
                "id": self.ccl.vmachine.list()[0],
            }
        else:
            data = self.pcl.actors.cloudbroker.machine.prepareForMigration(
                newspace.id, vmdata
            )
        ...
        if not self.dryrun:
            ...
            self.source_ccl.disk.updateSearch(
                {"id": {"$in": vm.disks}}, {"$set": {"status": "DELETED"}}
            )
            self.ccl.vmachine.updateSearch(
                {"id": newvm.id}, {"$set": {"status": "RUNNING"}}
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-a",
        "--accounts",
        help="Comma seperated accountids that require migration",
        required=True,
    )
    parser.add_argument(
        "-c",
        ...
        dest="cloudspaces",
        default=None,
        help="Filter for cloudspaces to migrate",
    )
    parser.add_argument(
        "-m", "--vm", dest="vms", default=None, help="Filter for vms to migrate"
    )
    parser.add_argument(
        "-d", "--dry-run", dest="dry", action="store_true", default=False
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Amount of VMs to migrate at once",
    )
    parser.add_argument(
        "-s",
        "--status",
    ...
    migrator = Migrator(
        options.debug, options.dry, options.concurrency, cloudspaces, vms
    )
