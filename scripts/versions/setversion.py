...
parser = OptionParser()
parser.add_option(
    "-d", "--directory", dest="directory", help="directory (ays repository) to scan"
)
parser.add_option(
    "-o",
    "--repository",
    dest="repository",
    help="repository url (eg: git.aydo.com/0-complexity/openvcloud.git)",
)

parser.add_option("-b", "--branch", dest="branch", help="freeze to than branch name")
parser.add_option(
    "-r", "--revision", dest="revision", help="freeze to that revision (commit)"
)
...
target = {"branch": options.branch, "revision": options.revision, "tag": options.tag}
