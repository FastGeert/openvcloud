...
def create_user(name, company, emails, password, companyurl, displayname, creationTime):
    ...
    create_user_request_params = dict(
        ...
        customfields=base64.b64encode(
            phpserialize.dumps([companyurl, displayname, creationTime])
        ),
        noemail=True,
        skipvalidation=True,
    )
...
def list_users():
    ...
    list_users_request_params = dict(
        action="getclients", limitnum=10000000, responsetype="json"
    )
...
def delete_user(userid):
    delete_users_request_params = dict(
        action="deleteclient", clientid=userid, responsetype="json"
    )
