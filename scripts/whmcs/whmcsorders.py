...
def accept_order(orderId):
    request_params = dict(action="acceptorder", orderid=orderId, sendemail=False)
...
def add_order(userId, productId, name, cloudbrokerId, location):
    ...
    request_params = dict(
        ...
        customfields=base64.b64encode(
            phpserialize.dumps([cloudbrokerId, name, location])
        ),
        noemail=True,
        skipvalidation=True,
        responsetype="json",
...
def list_orders(orderId=None):
    ...
    request_params = dict(action="getorders", limitnum=10000000, responsetype="json")
...
def delete_order(orderId):
    request_params = dict(action="deleteorder", orderid=orderId, responsetype="json")
