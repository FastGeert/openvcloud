...
class Vnas(object):
    ...
    def _format_ays_cmd(
        self, action, service_name, service_instance="main", args={}, parent=""
    ):
        """
    ...
    def create_vm(self, name, memsize=2, ssdsize=10, stack_id=None, disks=[]):
        ...
        id, ip, port = self.ovc.createMachine(
            self.spacesecret,
            name,
            memsize=memsize,
            ssdsize=ssdsize,
    ...
    def connect(self, username, password, cloudspace, location="dev"):
        ...
        """
        self.spacesecret = self.ovc.getCloudspaceSecret(
            username, password, cloudspace, location
        )
    ...
    def create_master(self):
        ...
        self.config_vm(cl)
        data = {"instance.param.rootpasswd": "rooter", "instance.ip": ip}
        ...
        return {
            ...
            "port": obj["interfaces"][0]["ipAddress"],
        }
    ...
    def create_backend(self, id, stack_id, master_ip, nid, nbr_disk=10):
        ...
        ip, port, passwd = self.create_vm(
            vmName, memsize=4, stack_id=stack_id, disks=[2000 for _ in range(nbr_disk)]
        )
        cl = self.ssh_to_vm(ip, port=port, passwd=passwd)
        self.config_vm(cl)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--api",
        required=True,
        help="url of the environement api. (du-conv-1.demo.greenitglobe.com)",
    )
    ...
    if args.action == "deploy":
        ...
        if not ok:
            ...
            state.done(
                "master", {"ip": master_ip, "key": vnas.masterkey, "port": master_port}
            )
        ...
        for i in range(2):
            ...
            if not ok:
                store = vnas.create_frontend(
                    i, i + 1, ad_ip, master_ip, i + 1, backends
                )
