...
def install(node, login, passwd, url):
    cl = j.ssh.connect(
        addr=node["ip"], port=22, passwd=node["passwd"], verbose=True, keypath=None
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-u",
        "--url",
        help="url to the bootstrapp\n",
        default="https://bootstrapscaleout1.demo.greenitglobe.com/",
        required=True,
    )
    parser.add_argument(
        "-n",
        "--nodes",
        help="path to a json file containing the nodes infos",
        default="nodes.json",
        required=True,
    )
