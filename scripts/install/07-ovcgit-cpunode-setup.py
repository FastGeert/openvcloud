...
parser.add_argument(
    "-s",
    "--skip-proxy-config",
    dest="skip_proxy_config",
    help="skip proxy configuration",
    default=False,
    action="store_true",
)
...
backplane1 = node.execute(
    "ip -4 -o addr show dev backplane1 | awk '{ print $4 }' | cut -d'/' -f 1"
)
...
vncurl = "https://novnc-%s.%s" % (
    node.parent.instance,
    settings.getStr("instance.ovc.domain"),
)
data_cpu = {
