...
parser.add_argument(
    ...
    dest="type",
    default="storagenode",
    help="Node type storagenode or storagedriver",
)
...
configure = j.application.getAppInstanceHRD(
    name="ovc_configure_aio", instance="main", domain="openvcloud"
)
...
j.console.info(
    "storagenode cloudspace       : %s" % data_cpu["instance.param.master.addr"]
)
...
for storagetype in options.type:
    packagename = (
        "cb_storagenode_aio" if storagetype == "storagenode" else "cb_storagedriver_aio"
    )
...
if "storagedriver" in options.type:
    ...
    if "MASTER" in node.execute(
        ...
        oauth_authorize_uri = os.path.join(
            ovc_iyo.actions.baseurl, "v1/oauth/authorize"
        )

        data_oauth = {
            "instance.oauth.id": ovc_iyo.actions.client_id,
        ...
        temp = j.atyourservice.new(
            name="openvstorage_oauth", instance="main", args=data_oauth, parent=node
        )
        temp.consume("node", node.instance)
        temp.install(deps=True)
...
j.system.process.execute(
    "jspython /opt/code/github/0-complexity/openvcloud/scripts/proxy/update-config.py",
    True,
    True,
)
