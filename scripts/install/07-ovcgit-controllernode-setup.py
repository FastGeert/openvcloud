...
ssl = j.application.getAppInstanceHRD(
    name="ssloffloader", instance="main", domain="openvcloud", parent=ovcproxy
)
...
if len(refsrv) > 0:
    ...
    autossh = next(
        iter(j.atyourservice.findServices(name="autossh", instance=nodename)), None
    )
    if not autossh:
    ...
    temp = j.atyourservice.new(
        name="autossh", instance="http_grafana", args=data_autossh, parent=node
    )
    temp.consume("node", node.instance)
    temp.install(deps=True)
    dashboard_data = {
        "instance.datasource.ip": refaddress,
    ...
    mgmtip = node.execute(
        "ip -4 -o addr show dev mgmt | awk 'NR==1 { print $4 }' | cut -d'/' -f 1"
    )
    dashboard_data = {
        "instance.datasource.ip": mgmtip,
