...
parser.add_option(
    "-e", "--environment", dest="environment", help="environment name (eg: be-test-1)"
)
parser.add_option(
    "-d", "--domain", dest="domain", help="domain name (default: demo.greenitglobe.com)"
)
...
"""
# global settings
optlist = {"environment": options.environment}
...
if options.backend == "ms1":
    ...
    machine = ovc.initMothership(
        options.user, options.passwd, options.location, options.cloudspace
    )
    machine["type"] = "ms1"

# docker
if options.backend == "docker":
    optlist = {"remote": options.remote, "port": options.port, "public": options.public}
