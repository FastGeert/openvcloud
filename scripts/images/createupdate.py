...
def downloadImage(jp, url):
    destination = j.system.fs.joinPaths(
        jp.getPathFilesPlatform("generic"), "root", "mnt", "vmstor", "templates"
    )
...
def writeConfigure(jp, name, description, imagepath, type_):
    ...
    result = template.render(
        type=type_, size=args.size, name=name, description=description, imagepath=image
    )
    configurepath = j.system.fs.joinPaths(
        jp.getPathMetadata(), "actions", "install.configure.py"
    )
