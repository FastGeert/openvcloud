...
parser.add_option(
    ...
    action="store_true",
    dest="self",
    help="only update the update script",
)
parser.add_option(
    "-r",
    "--restart",
...
parser.add_option(
    ...
    action="store_true",
    dest="restartNodes",
    help="only restart all the nodes",
)
parser.add_option(
    "-N",
    "--restart-cloud",
    action="store_true",
    dest="restartCloud",
    help="only restart all the cloudspace vm",
)
...
group.add_option(
    "--branch-ovc", dest="branch_ovc", help="Branch to update OpenvCloud to"
)
group.add_option(
    "--branch-ovc-core",
    dest="branch_ovc_core",
    default=None,
    help="Branch to update OpenvCloud to",
)
group.add_option(
    "--branch-ovc-selfhealing",
    dest="branch_ovc_selfhealing",
    default=None,
    help="Branch to update OpenvCloud to",
)
parser.add_option(
...
parser.add_option(
    ...
    action="store_true",
    dest="commit",
    help="commit the ovcgit repository",
)
...
def update(nodessh):
    ...
    try:
        nodessh.execute(
            _get_update_cmd("jumpscale7", "*", options.branch_js, options.tag_js)
        )
        nodessh.execute(
            _get_update_cmd(
                "0-complexity", "openvcloud", options.branch_ovc_core, options.tag_ovc
            )
        )
        nodessh.execute(
            _get_update_cmd(
                "0-complexity",
                "selfhealing",
                options.branch_ovc_selfhealing,
                options.tag_ovc,
            )
        )
        nodessh.execute(
            _get_update_cmd(
                "0-complexity", "g8vdc", options.branch_ovc, options.tag_ovc
            )
        )
        nodessh.execute(
            _get_update_cmd(
                "0-complexity", "openvcloud_ays", options.branch_ovc, options.tag_ovc
            )
        )
    ...
    for version in versions:
        for hostname in versions[version]:
            for hostmatch in versions[version]:
                ...
                # version mismatch
                if (
                    versions[version][hostmatch]["name"]
                    != versions[version][hostname]["name"]
                ):
...
def applyOnServices(services, func, msg=None, kwargs=None):
    ...
    def schedule_services():
        while len(runningjobs) < concurrency and servicestostart:
            ...
            proc = multiprocessing.Process(
                target=func, args=(startme,), kwargs=kwargs or {}
            )
            proc.start()
    ...
    for job in alljobs:
        ...
        if job.exitcode:
            ...
            j.console.warning(
                "Failed to execute {} on {}".format(funcname, job.service)
            )
            error = True
    if error:
        sys.exit(1)
...
def updateLocal():
    ...
    with j.logger.nostdout():
        j.do.execute(
            _get_update_cmd("jumpscale", "*", options.branch_js, options.tag_js)
        )
        j.do.execute(
            _get_update_cmd(
                "0-complexity", "openvcloud", options.branch_ovc, options.tag_ovc
            )
        )
        j.do.execute(
            _get_update_cmd(
...
def cleanupLogs():
    ...
    def cleanlogs(master):
        master.execute(
            "jspython /opt/code/github/0-complexity/openvcloud/scripts/updates/cleanlogs.py -s {}".format(
                starttime
            )
        )
...
def versionBuilder():
    ...
    settings = j.application.getAppInstanceHRD(
        name="ovc_setup", instance="main", domain="openvcloud"
    )
...
def updateGit():
    ...
    with j.logger.nostdout():
        ...
        j.do.execute(
            "cd %s; git commit -m 'environement updated (update script)'" % repopath
        )
        j.do.execute("cd %s; git push" % repopath)
