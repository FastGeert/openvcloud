...
parser = OptionParser()
parser.add_option(
    "-c", "--check", action="store_true", dest="check", help="check for update"
)
parser.add_option(
    "-u",
    "--update",
    action="store_true",
    dest="update",
    help="just run apt-get update on each nodes",
)
parser.add_option(
    "-P",
    "--preprocess",
    action="store_true",
    dest="prepro",
    help="runs some pre-upgrade commands",
)
parser.add_option(
    "-S",
    "--postprocess",
    action="store_true",
    dest="postpro",
    help="runs some post-upgrade commands",
)
parser.add_option(
    "-r",
    "--arakoon",
    action="store_true",
    dest="arakoon",
    help="attempt to update only arakoon",
)
parser.add_option(
    "-o",
    "--framework",
    action="store_true",
    dest="framework",
    help="attempt to update only ovs-framework",
)
parser.add_option(
    "-v",
    "--volumedriver",
    action="store_true",
    dest="volumedriver",
    help="attempt to update only volumedriver",
)
parser.add_option(
    "-a",
    "--alba",
    action="store_true",
    dest="alba",
    help="attempt to update only alba backend",
)
parser.add_option(
    "-s",
    "--sync",
    action="store_true",
    dest="sync",
    help="sync apt-get archives directory (download faster)",
)
parser.add_option(
    "-O", "--stop-vm", action="store_true", dest="stopVM", help="stop all running vm"
)
parser.add_option(
    "-A",
    "--start-vm",
    action="store_true",
    dest="startVM",
    help="start previously stopped vm",
)
parser.add_option(
    "-k",
    "--skip-vm",
    action="store_true",
    dest="skip",
    help="do not start/stop vm during update",
)
...
def getGenericFiles(nodes, path, extstrip):
    for ns in nodes:
        if ns.instance not in hosts:
            ...
            command = "(basename -s %s -a $(ls %s)) 2> /dev/null; exit 0" % (
                extstrip,
                path,
            )
...
def pidFromStatus(singleNode, service):
    return singleNode.execute(
        "status %s | grep 'running' | awk '{ print $4 }'" % service
    )
...
def framework(nodes):
    ...
    for node in nodes:
        executeOnNodes(
            [node],
            "python /opt/code/git/0-complexity/openvcloud/scripts/ovs/post-upgrade-all.py",
        )

    for node in nodes:
        executeOnNodes(
            [node],
            "python /opt/code/git/0-complexity/openvcloud/scripts/ovs/post-upgrade-single.py",
        )
        break
    ...
    for node in nodes:
        ...
        executeOnNodes(
            [node],
            "sed -i.bak \"s/^ALLOWED_HOSTS.*$/ALLOWED_HOSTS = ['*']/\" /opt/OpenvStorage/webapps/api/settings.py",
        )
...
def volumedriver(nodes):
    for node in nodes:
        ...
        executeOnNodes(
            [node],
            "apt-get install -y --force-yes volumedriver-base volumedriver-server",
        )
...
def arakoon(nodes):
    ...
    for cluster in clusters:
        executeOnNodes(
            [node],
            "python %s --cluster %s --address %s" % (upscript, cluster, backplane),
        )
